...
const Predict = () => {
  ...
  function applyFromPayload(obj: any) {
    ...
    // dev/pubs (prefer arrays, fallback to single)
    const devs =
      Array.isArray(obj.developers) && obj.developers.length
        ? obj.developers
        : obj.developer
        ? [obj.developer]
        : ['dev__Other'];
    ...
    setDevelopers(devs);
    setPublishers(pubs);
    update('developers', devs);
    update('publishers', pubs);
    ...
    // platforms were set above via windows/mac/linux
  }
  ...
  function applyFromDatasetEntry(entry: any) {
    ...
    // developers/publishers from one-hots == 1
    const pickedDevs: string[] = [];
    const pickedPubs: string[] = [];

    for (const [key, rawVal] of Object.entries(entry)) {
      if (rawVal !== 1 && rawVal !== 1.0) continue;

      if (key.startsWith('dev__')) {
        const namePart = key.slice(5); // remove 'dev__'
        pickedDevs.push('dev__' + sanitizeNameToFeature(namePart));
      }
      if (key.startsWith('pub__')) {
        const namePart = key.slice(5);
        const cleaned = sanitizeNameToFeature(namePart);
        if (cleaned) pickedPubs.push('pub__' + cleaned);
      }
    }

    const devs = pickedDevs.length ? pickedDevs : ['dev__Other'];
    const pubs = pickedPubs.length ? pickedPubs : ['pub__Other'];
    setDevelopers(devs);
    setPublishers(pubs);
    update('developers', devs);
    update('publishers', pubs);
